...
static u64 calc_time_slice(task_ctx *taskc, struct cpu_ctx *cpuc)
{
	...
	/*
	 * If a TLD hint is set, use it directly and skip all other
	 * slice calculation logic.
	 */
	if (taskc->tld_hint_slice_ns) {
		taskc->slice_wall = taskc->tld_hint_slice_ns;
...
void BPF_STRUCT_OPS(lavd_tick, struct task_struct *p)
{
	...
	/*
	 * Enforce the TLD slice hint as a target. This must be last so it
	 * is not overwritten by preemption or pinned-task shrinking above.
	 */
	if (taskc->tld_hint_slice_ns) {
