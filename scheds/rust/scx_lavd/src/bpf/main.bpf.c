...
void BPF_STRUCT_OPS(lavd_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	cpdom_id = pick_proper_dsq(p, taskc, task_cpu, &cpu, &is_idle, cpuc_cur);
	...
	if (!shrinked && !no_preemption)
		try_find_and_kick_victim_cpu(p, taskc, cpu, cpu_to_dsq(cpu));
}
...
void BPF_STRUCT_OPS(lavd_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	s32 new_cpu;
	bool try_consume;
	u64 dsq_ids[LAVD_DSQ_NUM_TYPES];
	 ...
	 */
	try_consume = false;
	bpf_for(dsq_type, dsq_start, LAVD_DSQ_NUM_TYPES) {
